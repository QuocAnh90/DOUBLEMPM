...
writerObj2           = VideoWriter('consolidation_old.avi');
...
%         [v_ssp,x_sp,F_sp,V_sp,s_sp,p_sp,v_wp,x_wp,pore_wp,V_wp] = double_MPM_solver_new(CModel,CModel_parameter,nodeCount,wpCount,spCount,cellCount,...
%         x_wp,x_sp,x_wpo,x_spo,d_wp,d_sp,le,NN,LOC,...
%         b_sp,b_wp,V_sp,ptraction_sp,ptraction_wp,v_ssp,v_wp,s_sp,pore_wp,m_wp,m_sp,p_sp,...
%         nfbcx,nfbcy,fbcx,fbcy,Kw,k,F_sp,V_spo,V_wp,dt,psp,pwp);
 ...
 for wp=1:wpCount
 pore_wp_ana(wp)=1/mv  *  (log(1+(exp(mv*ppp)-1)*pore_wp_ana(wp)));
%  pore_wp_ana(wp)=    4*ppp/pi*pore_wp_ana(wp);
 end
