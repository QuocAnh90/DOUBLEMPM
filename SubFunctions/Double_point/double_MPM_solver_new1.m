function[v_ssp,x_sp,F_sp,V_sp,s_sp,p_sp,v_wp,x_wp,pore_wp,V_wp] = double_MPM_solver_new1(CModel,CModel_parameter,nodeCount,wpCount,spCount,cellCount,...
...
% Liquid variables
[nmass_wi,nmomentum_wi,niforce_cell,neforce_wi,traction_wi]=Interpolate_Liquid_Particle_To_Grid_new1(...
NODES_wp,nodeCount,wpCONNECT,mwpoints,wpElems,LOCC,LOC,le,Nw,wpCount,b_wp,V_wp,ptraction_wp,v_wp,pore_wp,m_wp);
...
nvelo_wi            = zeros(nodeCount,2);
 ...
 nforce_wi =  n_i .* niforce_cell + neforce_wi + traction_wi - n_i.*ndrag;
 ...
 for n = 1:nodeCount
%  if nvolume_si(n) ~=0
 nforce_si(n,:) = niforce_si(n,:) + niforce_cell(n,:) +  neforce_si(n,:) + neforce_wi(n,:) + traction_wi(n,:) + traction_si(n,:) - nforce_wi(n,:);
%  end
 end
