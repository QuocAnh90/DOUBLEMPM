function[v_ssp,x_sp,F_sp,V_sp,s_sp,p_sp,v_wp,x_wp,pore_wp,F_wp,V_wp]...
...
%% Store particles shape function into cell
% Solid
...
% Liquid variables
[nmass_wi,nmomentum_wi,niforce_wi,neforce_wi,traction_wi]=Interpolate_Liquid_Particle_To_Grid(...
...
% Node variabless
n_i                 = ones(nodeCount,1);                    % Nodal porosity
k_i                 = zeros(nodeCount,1);                   % Nodal permeability
ndrag               = zeros(nodeCount,2);                   % Nodal Drag force
nvelo_si            = zeros(nodeCount,2);                   % Nodal solid velocity
nvelo_wi            = zeros(nodeCount,2);                   % Nodal liquid velocity
 ...
 % Nodal permeability calculation
 k_i(n)            = k;
 
 % Nodal Solid velocity calculation
 if nmass_si(n) ~=0
 nvelo_si(n,:)     = nmomentum_si(n,:)/nmass_si(n);
 end
 
 % Nodal Liquid velocity calculation
 if nmass_wi(n) ~=0
 nvelo_wi(n,:)     = nmomentum_wi(n,:)/nmass_wi(n);
 end
 
 % Nodal Drag forces calculation
 if nvolume_si(n) ~=0
 ...
 %% Update momentum
 nacc_si              = zeros(nodeCount,2);                 % Nodal Solid Acceleration
...
[F_wp,pore_wp] = Update_Pore_Pressure(...
