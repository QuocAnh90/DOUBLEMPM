...
NN(1)                 = 4;                              % number of nodes in X direction
NN(2)                 = 6;                             % number of nodes in Y direction
le(1)                 = 1;                           % size of element in X direction
le(2)                 = 1;                          % size of element in Y direction
...
% Solid particles
particle_per_cell       = 1;
spCount                 = 2*particle_per_cell;
lsp(1,1)                 = le(1);                                 % size of particle in X direction
lsp(1,2)                 = le(2);                              % size of particle in Y direction
...
while sp<spCount+0.0001
    for i=1:2
        for j=1:1
            x_sp(sp,1:2)= [1*le(1,1)+0.5*lsp(1,1)+(j-1)*lsp(1,1) 1*le(1,2)+0.5*lsp(1,2)+(i-1)*lsp(1,2)];
            sp=sp+1;
        end
    end
end
...
% Liquid particles
wpCount                 = 2*particle_per_cell;
lwp(1,1)                 = le(1);                                 % size of particle in X direction
lwp(1,2)                 = le(2);                              % size of particle in Y direction
...
while wp<wpCount+0.0001
    for i=1:2
        for j=1:1
            x_wp(wp,1:2)= [1*le(1,1)+0.5*lwp(1,1)+(j-1)*lwp(1,1) 1*le(1,2)+0.5*lwp(1,2)+(i-1)*lwp(1,2)];
            wp=wp+1;
        end
    end
end
